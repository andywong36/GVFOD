...
Check the end of this file for code related to TimeSeriesFolds
"""
...
from .model_selection import TimeSeriesFolds

cvs.append(TimeSeriesFolds)
...
# Plot TimeSeriesFolds
this_cv = TimeSeriesFolds(n_splits=4, min_test_size=30, max_test_size=30, delay=10, min_train_size=15, max_train_size=None)
fig, ax = plt.subplots(figsize=(6, 3))
plot_cv_indices(this_cv, X, y, groups, ax, n_splits)
ax.legend([Patch(color=cmap_cv(.8)), Patch(color=cmap_cv(.02))],
          ['Testing set', 'Training set'], loc=(1.02, .8))
